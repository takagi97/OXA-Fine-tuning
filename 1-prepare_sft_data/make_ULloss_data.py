...
# === Configuration Area ===
INPUT_PATH = "AceReason-1.1-Math.uuid.gt.verify.true.dps_7b_sample_list.dps_7b_sample_xverifier.false.dps_7b_sample_Qwen2.5-Math-7B_ppl_list_all.add_think.rm_request_fail.rm_len_25k.jsonl"
LOWEST_OUT_PATH = "AceReason-1.1-Math.uuid.gt.verify.true.dps_7b_sample_list.dps_7b_sample_xverifier.false.dps_7b_sample_Qwen2.5-Math-7B_ppl_list_all.add_think.rm_request_fail.rm_len_25k.lowest_50000.1query1response.jsonl"

GLOBAL_K = 50000  # Globally select the lowest 50000 Responses by PPL
MAX_RESPONSES_PER_QUERY = 1
MIN_PPL_THRESHOLD = 1.2  # [New] PPL lower limit, responses with value lower than this will be filtered out

PPL_KEY = "dps_7b_sample_Qwen2.5-Math-7B_ppl_list"
RESPONSE_KEY = "dps_7b_sample_list" 
# =================
...
def main():
    ...
    skipped = 0
    filtered_by_min_ppl_count = 0 # Count of items filtered by the lower limit
    ...
    print(f"[CONFIG] PPL minimum threshold: {MIN_PPL_THRESHOLD} (values lower than this will be discarded)")
        ...
        for line in fin:
            ...
            for i in range(len(ppl_list)):
                try:
                    ...
                    # [New Logic] Check PPL lower limit
                    if p < MIN_PPL_THRESHOLD:
                        filtered_by_min_ppl_count += 1
                        continue
    ...
    print(f"[INFO] Number of Responses filtered due to PPL < {MIN_PPL_THRESHOLD}: {filtered_by_min_ppl_count}")
