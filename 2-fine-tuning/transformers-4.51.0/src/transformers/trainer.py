...
from torch.utils.data import DataLoader, Dataset, IterableDataset, RandomSampler, SequentialSampler, BatchSampler
...
# 专门用于SFT+UL的sampler
class MixedLossTypeGlobalBatchSampler(BatchSampler):
    """
    目标：在“全局 batch（所有卡合起来）”里，放入 k_ul_global 个 UL，其余为 CE。
    然后对该全局 batch 按 rank 切片，得到每卡的本地 batch（大小=per_device_batch_size）。

    约定: dataset[i]["loss_type"] in {0,1}，0=CE, 1=UL。

    语义：
    - global_bs = per_device_batch_size * world_size
    - 每个全局 batch：UL = k_ul_global, CE = global_bs - k_ul_global
    - drop_last=True：严格满配，不能满足任何一类的全局需求则本轮结束，不产出不满的 batch
    - drop_last=False：允许“回绕一次”；当第二类也需要回绕时，产出一个“最后的（可能不满）全局 batch”并结束

    分布式：
    - 每个 rank 上都构造相同的全局 batch 序列（保持相同 rng/seed），
      然后本 rank 取切片 [rank*per_dev_bs : (rank+1)*per_dev_bs]
    - 因此所有 rank 在同一次 global step 上处理的是“同一个全局 batch”的不同切片，
      从而保证“全局 UL 数量 == k_ul_global”。

    注意：
    - 本地（per-rank）batch 的 UL 个数会随切片分布而变化，不再等于 k_ul_global。
    """
    ...
    def __init__(
        ...
        assert per_device_batch_size > 0, "per_device_batch_size must be > 0"
        assert world_size >= 1, "world_size must be >= 1"
        global_bs = per_device_batch_size * world_size
        assert 0 <= k_ul_global <= global_bs, "k_ul_global must be in [0, per_device_batch_size*world_size]"

        self.dataset = dataset
        self.per_device_batch_size = per_device_batch_size
        self.k_ul_global = k_ul_global
        ...
        self.base_seed = int(seed if seed is not None else 42)
        ...
        for i in range(len(dataset)):
            item = dataset[i]
            lt = item["loss_type"] if isinstance(item, dict) else getattr(item, "loss_type")
            (ul_idx if int(lt) == 1 else ce_idx).append(i)

        # 语义检查
        if self.k_ul_global > 0 and len(ul_idx) == 0:
            raise ValueError("k_ul_global > 0 but no UL samples were found in dataset.")
        if (self.global_bs - self.k_ul_global) > 0 and len(ce_idx) == 0:
            raise ValueError("global CE needed > 0 but no CE samples were found in dataset.")
    ...
    def set_epoch(self, epoch: int):
        self.rng.seed(self.base_seed + int(epoch))

    def _get_num_global_batches(self) -> int:
        """
        估计“全局 batch”的数量（对 drop_last=True 是准确的；对 drop_last=False 是保守估计）。
        """
        need_ul = self.k_ul_global
        need_ce = self.global_bs - self.k_ul_global

        if need_ul == 0 and need_ce == 0:
            return 0  # 退化情况

        if need_ul == 0:
            n = len(self.ce_idx) // need_ce if need_ce > 0 else 0
            if not self.drop_last and (need_ce > 0) and (len(self.ce_idx) % need_ce != 0):
                n += 1
            return n

        if need_ce == 0:
            n = len(self.ul_idx) // need_ul if need_ul > 0 else 0
            if not self.drop_last and (need_ul > 0) and (len(self.ul_idx) % need_ul != 0):
                n += 1
            return n

        # 两类都需要
        n_ul = len(self.ul_idx) // need_ul
        n_ce = len(self.ce_idx) // need_ce
        n = min(n_ul, n_ce)
        if not self.drop_last:
            rem = (len(self.ul_idx) % need_ul != 0) or (len(self.ce_idx) % need_ce != 0)
            if rem:
                n += 1
        return n

    def __len__(self) -> int:
        # 每个 rank 都会产出与全局 batch 数相同的“本地步数”
        return self._get_num_global_batches()

    def __iter__(self):
        ce_pool = self.ce_idx[:]
        ul_pool = self.ul_idx[:]
        if self.shuffle:
            self.rng.shuffle(ce_pool)
            self.rng.shuffle(ul_pool)
        ...
        while True:
            ...
            # ---- drop_last=True：必须满配全局批次 ----
            if self.drop_last:
                can_ul_full = (need_ul == 0) or (ul_ptr + need_ul <= len(ul_pool))
                can_ce_full = (need_ce == 0) or (ce_ptr + need_ce <= len(ce_pool))
                if not (can_ul_full and can_ce_full):
                    break

                global_batch = []
                if need_ul > 0:
                    global_batch.extend(ul_pool[ul_ptr:ul_ptr + need_ul])
                    ul_ptr += need_ul
                if need_ce > 0:
                    global_batch.extend(ce_pool[ce_ptr:ce_ptr + need_ce])
                    ce_ptr += need_ce

                # 可选：打乱 global batch 内部顺序
                if self.shuffle:
                    self.rng.shuffle(global_batch)

                # 切片到本 rank
                start = self.rank * self.per_device_batch_size
                end = start + self.per_device_batch_size
                local_batch = global_batch[start:end]
                yield local_batch
                continue

            # ---- drop_last=False：允许回绕一次；第二类也耗尽则产出最后（可能不满）的全局批次并结束 ----
            ce_short = (need_ce > 0) and (ce_ptr + need_ce > len(ce_pool))
            ul_short = (need_ul > 0) and (ul_ptr + need_ul > len(ul_pool))

            # 只需要一种类型
            if need_ul == 0 and need_ce > 0:
                # 不回绕：把剩余 CE 分批出完
                if ce_ptr >= len(ce_pool):
                    break
                take_ce = min(need_ce, len(ce_pool) - ce_ptr)
                global_batch = ce_pool[ce_ptr:ce_ptr + take_ce]
                ce_ptr += take_ce
                # 补不满就直接作为最后一批
                if self.shuffle:
                    self.rng.shuffle(global_batch)
                ...
                local_batch = global_batch[start:end] if len(global_batch) >= end else global_batch[start:len(global_batch)]
            ...
            if need_ce == 0 and need_ul > 0:
                if ul_ptr >= len(ul_pool):
                    break
                take_ul = min(need_ul, len(ul_pool) - ul_ptr)
                global_batch = ul_pool[ul_ptr:ul_ptr + take_ul]
                ul_ptr += take_ul
                if self.shuffle:
                    self.rng.shuffle(global_batch)
                start = self.rank * self.per_device_batch_size
            ...
            # 两类都需要
            if ce_short and ul_short:
                # 两类都不够：把剩余全部拿完，拼成最后一个不满的全局 batch
                take_ul = len(ul_pool) - ul_ptr
                take_ce = len(ce_pool) - ce_ptr
                if take_ul == 0 and take_ce == 0:
                    break
                global_batch = []
                if take_ul > 0:
                    global_batch.extend(ul_pool[ul_ptr:ul_ptr + take_ul])
                    ul_ptr += take_ul
                if take_ce > 0:
                    global_batch.extend(ce_pool[ce_ptr:ce_ptr + take_ce])
                    ce_ptr += take_ce
                if self.shuffle:
                    self.rng.shuffle(global_batch)
                start = self.rank * self.per_device_batch_size
            ...
            if ce_short and not ul_short:
                # 如果 UL 已经回绕过，则 CE 成为第二个耗尽的 -> 最后一批
                if ul_wrapped:
                    take_ul = min(need_ul, len(ul_pool) - ul_ptr)
                    take_ce = min(need_ce, len(ce_pool) - ce_ptr)
                    if take_ul == 0 and take_ce == 0:
                    ...
                    global_batch = []
                    if take_ul > 0:
                        global_batch.extend(ul_pool[ul_ptr:ul_ptr + take_ul]); ul_ptr += take_ul
                    if take_ce > 0:
                        global_batch.extend(ce_pool[ce_ptr:ce_ptr + take_ce]); ce_ptr += take_ce
                    if self.shuffle:
                        self.rng.shuffle(global_batch)
                    start = self.rank * self.per_device_batch_size
                    end = start + self.per_device_batch_size
            ...
            global_batch = []
            if need_ul > 0:
                global_batch.extend(ul_pool[ul_ptr:ul_ptr + need_ul]); ul_ptr += need_ul
            if need_ce > 0:
                global_batch.extend(ce_pool[ce_ptr:ce_ptr + need_ce]); ce_ptr += need_ce
            if self.shuffle:
                self.rng.shuffle(global_batch)
            ...
            local_batch = global_batch[start:end]
            # 这里 local_batch 长度应等于 per_device_batch_size（除非进入了“不满”的分支）
            if len(local_batch) == 0:
                break
            yield local_batch
...
class Trainer:
    ...
    def __init__(
        ...
        self._ul_global = self.args.ul_global
        self._all_global = self._train_batch_size * self.accelerator.num_processes
        self._ce_global = self._all_global - self._ul_global
    ...
    def _remove_unused_columns(self, dataset: "datasets.Dataset", description: Optional[str] = None):
        ...
        if "loss_type" not in signature_columns:
            signature_columns.append("loss_type")
    ...
    def get_train_dataloader(self) -> DataLoader:
        ...
        train_dataset = self.train_dataset
        data_collator = self.data_collator
        # 和原版 Trainer 一样，先去掉未用列
        if is_datasets_available() and isinstance(train_dataset, datasets.Dataset):
            train_dataset = self._remove_unused_columns(train_dataset, description="training")
        else:
            data_collator = self._get_collator_with_removed_columns(data_collator, description="training")

        batch_sampler = MixedLossTypeGlobalBatchSampler(
            dataset=train_dataset,
            per_device_batch_size=self._train_batch_size,      # 每张卡的 batch size
            k_ul_global=self._ul_global,                     # 全局 UL 数量（抛去梯度累积的batch size）
            drop_last=True, # 不这么设置的话会导致训练卡死
            shuffle=True,
            seed=42, # int(self.args.seed),
    ...
    def _inner_training_loop(
        ...
        tr_ce_data_entropy = torch.tensor(0.0, device=args.device)
        tr_ul_data_entropy = torch.tensor(0.0, device=args.device)
        tr_ce_loss = torch.tensor(0.0, device=args.device)
        tr_ul_loss = torch.tensor(0.0, device=args.device)
        tr_weighted_ul_loss = torch.tensor(0.0, device=args.device)
        ...
        for epoch in range(epochs_trained, num_train_epochs):
            ...
            for _ in range(total_updates):
                ...
                for i, inputs in enumerate(batch_samples):
                    ...
                    with context():
                        tr_loss_step, tr_ce_data_entropy_step, tr_ul_data_entropy_step, tr_ce_loss_step, tr_ul_loss_step, tr_weighted_ul_loss_step = self.training_step(model, inputs, num_items_in_batch)
                    ...
                    if (
                        ...
                        tr_ce_data_entropy = tr_ce_data_entropy + tr_ce_data_entropy / (1 + self.state.global_step - self._globalstep_last_logged)
                        tr_ul_data_entropy = tr_ul_data_entropy + tr_ul_data_entropy / (1 + self.state.global_step - self._globalstep_last_logged)
                        tr_ce_loss = tr_ce_loss + tr_ce_loss / (1 + self.state.global_step - self._globalstep_last_logged)
                        tr_ul_loss = tr_ul_loss + tr_ul_loss / (1 + self.state.global_step - self._globalstep_last_logged)
                        tr_weighted_ul_loss = tr_weighted_ul_loss + tr_weighted_ul_loss / (1 + self.state.global_step - self._globalstep_last_logged)
                    else:
                        ...
                        tr_ce_data_entropy = tr_ce_data_entropy + tr_ce_data_entropy_step
                        tr_ul_data_entropy = tr_ul_data_entropy + tr_ul_data_entropy_step
                        tr_ce_loss = tr_ce_loss + tr_ce_loss_step
                        tr_ul_loss = tr_ul_loss + tr_ul_loss_step
                        tr_weighted_ul_loss = tr_weighted_ul_loss + tr_weighted_ul_loss_step
                    ...
                    if do_sync_step:
                        ...
                        self._maybe_log_save_evaluate(
                            ...
                            tr_ce_data_entropy=tr_ce_data_entropy,
                            tr_ul_data_entropy=tr_ul_data_entropy,
                            tr_ce_loss=tr_ce_loss,
                            tr_ul_loss=tr_ul_loss,
                            tr_weighted_ul_loss=tr_weighted_ul_loss,
                        )
                    else:
            ...
            self._maybe_log_save_evaluate(
                tr_loss, grad_norm, model, trial, epoch, ignore_keys_for_eval, start_time, learning_rate=learning_rate, tr_ce_data_entropy=tr_ce_data_entropy, tr_ul_data_entropy=tr_ul_data_entropy, tr_ce_loss=tr_ce_loss, tr_ul_loss=tr_ul_loss, tr_weighted_ul_loss=tr_weighted_ul_loss,
            )
    ...
    def _maybe_log_save_evaluate(
        self, tr_loss, grad_norm, model, trial, epoch, ignore_keys_for_eval, start_time, learning_rate=None, tr_ce_data_entropy=None, tr_ul_data_entropy=None, tr_ce_loss=None, tr_ul_loss=None, tr_weighted_ul_loss=None,
    ):
        if self.control.should_log and self.state.global_step > self._globalstep_last_logged:
            ...
            tr_ce_data_entropy_scalar = self._nested_gather(tr_ce_data_entropy).mean().item()
            tr_ul_data_entropy_scalar = self._nested_gather(tr_ul_data_entropy).mean().item()
            tr_ce_loss_scalar = self._nested_gather(tr_ce_loss).mean().item()
            tr_ul_loss_scalar = self._nested_gather(tr_ul_loss).mean().item()
            tr_weighted_ul_loss_scalar = self._nested_gather(tr_weighted_ul_loss).mean().item()
            ...
            logs["ce_loss"] = round(tr_ce_loss_scalar / (self.state.global_step - self._globalstep_last_logged), 4)
            logs["ul_loss"] = round(tr_ul_loss_scalar / (self.state.global_step - self._globalstep_last_logged), 4)
            logs["weighted_ul_loss"] = round(tr_weighted_ul_loss_scalar / (self.state.global_step - self._globalstep_last_logged), 4)
            logs["ce_data_entropy"] = round(tr_ce_data_entropy_scalar / (self.state.global_step - self._globalstep_last_logged), 4)
            logs["ul_data_entropy"] = round(tr_ul_data_entropy_scalar / (self.state.global_step - self._globalstep_last_logged), 4)
    ...
    def training_step(
        ...
        with self.compute_loss_context_manager():
            ...
            ce_data_entropy = getattr(outputs, "ce_data_entropy", torch.tensor(0.0, device=loss.device))
            ul_data_entropy = getattr(outputs, "ul_data_entropy", torch.tensor(0.0, device=loss.device))
            ce_loss = getattr(outputs, "ce_loss", torch.tensor(0.0, device=loss.device))
            ul_loss = getattr(outputs, "ul_loss", torch.tensor(0.0, device=loss.device))
            weighted_ul_loss = getattr(outputs, "weighted_ul_loss", torch.tensor(0.0, device=loss.device))
            del outputs
        ...
        if self.args.n_gpu > 1:
            ...
            ce_data_entropy = ce_data_entropy.mean()
            ul_data_entropy = ul_data_entropy.mean()
            ce_loss = ce_loss.mean()
            ul_loss = ul_loss.mean()
            weighted_ul_loss = weighted_ul_loss.mean()
        ...
        if self.use_apex:
            ...
            loss = loss / self.args.gradient_accumulation_steps
            ce_data_entropy = ce_data_entropy / self.args.gradient_accumulation_steps
            ul_data_entropy = ul_data_entropy / self.args.gradient_accumulation_steps
            ce_loss = ce_loss / self.args.gradient_accumulation_steps
            ul_loss = ul_loss / self.args.gradient_accumulation_steps
            weighted_ul_loss = weighted_ul_loss / self.args.gradient_accumulation_steps
            ...
            return loss.detach(), ce_data_entropy, ul_data_entropy, ce_loss, ul_loss, weighted_ul_loss
    ...
    def compute_loss(
        ...
        labels: Optional[torch.Tensor] = inputs.pop("labels", None)
        loss_type: Optional[torch.Tensor] = inputs.pop("loss_type", None)  # 0=CE, 1=UL
        if self.model_accepts_loss_kwargs and num_items_in_batch is not None:
            inputs["num_items_in_batch"] = num_items_in_batch

        outputs = model(**inputs)

        if (loss_type is None) or (labels is None):
            loss = outputs["loss"] if isinstance(outputs, dict) else outputs[0]
            return (loss, outputs) if return_outputs else loss

        # 只要检查一下 batch 里是否包含 UL 样本即可决定走哪条分支
        ...
        # 把 padding 置为 ignore_index
        if "attention_mask" in inputs:
            labels = labels.clone()       # 后面 Trainer 还会用到 labels，不能就地改
            labels.masked_fill_(~inputs["attention_mask"].bool().to(device), ignore_index)

        ul_indices = (loss_type == 1).nonzero(as_tuple=True)[0]   # [] if none
        ce_indices = (loss_type == 0).nonzero(as_tuple=True)[0]
        
        if ul_indices.numel() == 0:
            # -------------- 只有 CE ------------------
            shifted = F.pad(labels[ce_indices], (0, 1), value=ignore_index)[..., 1:]
            flat_logits  = logits[ce_indices].view(-1, vocab_size)
            flat_targets = shifted.view(-1)
            ce_loss = F.cross_entropy(flat_logits, flat_targets,
                                ignore_index=ignore_index, reduction="mean")
            ul_loss = torch.tensor(0.0, device=device, dtype=ce_loss.dtype)
        else:
            # -------------- 只有 UL ------------------
            ul_logits = logits[ul_indices]
            ul_labels = labels[ul_indices]

            shifted_ul = F.pad(ul_labels, (0, 1), value=ignore_index)[..., 1:]
            flat_lbl   = shifted_ul.view(-1)
            valid_mask = (flat_lbl != ignore_index)

            log_probs = F.log_softmax(ul_logits, dim=-1).view(-1, vocab_size)
            tgt_lp = log_probs[torch.arange(log_probs.size(0), device=device), flat_lbl.clamp(min=0)]
            tgt_lp = tgt_lp[valid_mask]

            # 防止 p=1 ⇒ -log(1-p)=inf，给一点 ε
            eps       = 1e-6
            eps       = torch.tensor(-eps, device=tgt_lp.device)
            tgt_lp    = torch.clamp(tgt_lp, max=torch.log1p(eps))   # log(p) ≤ log(1-ε)

            t = -0.6931471805599453  # ln(0.5)
            ul_tok = torch.where(tgt_lp < t,
                                -torch.log1p(-torch.exp(tgt_lp)),
                                -torch.log(-torch.expm1(tgt_lp)))
            ul_loss = ul_tok.mean().to(logits.dtype)
            ce_loss = torch.tensor(0.0, device=device, dtype=ul_loss.dtype)
        # ---------- 组合 + scale ----------
        ul_loss_scaled = ul_loss / max(self._ul_global, 1.0)
        ce_loss_scaled = ce_loss / max(self._ce_global, 1.0)
        loss = ce_loss_scaled + self.args.ul_weight * ul_loss_scaled

        # 抵消 accelerator 的 batch 均值
        scale = self._all_global
        loss      = loss * scale
        ce_loss   = ce_loss_scaled * scale
        ul_loss   = ul_loss_scaled * scale

        outputs["ce_loss"]        = torch.tensor(ce_loss.item(), device=device, dtype=loss.dtype)
        outputs["ul_loss"]        = torch.tensor(ul_loss.item(), device=device, dtype=loss.dtype)
        outputs["weighted_ul_loss"] = torch.tensor(self.args.ul_weight * ul_loss.item(), device=device, dtype=loss.dtype)
    ...
    def create_accelerator_and_postprocess(self):
        ...
        args["dataloader_config"] = DataLoaderConfiguration(**{"split_batches": False, "dispatch_batches": None, "even_batches": False, "use_seedable_sampler": False, "non_blocking": False, "use_stateful_dataloader": False})
